...
for ($i = 1; $i <= 27; $i++){
	...
	file_put_contents('../store/html/' . $i . '.php', '<div><h5>' . $dateArray[$i-1] . '</h5><h1>' . $titleArray[$i-1] . '</h1></div>' . Markdown($thePost));
	file_put_contents('../store/look/' . $i . '.html', $a . '<title>in / ject // ' . $titleArray[$i-1] . '</title>' . $b . '<div><h5>'. $dateArray[$i-1] .'</h5><h1>'.$titleArray[$i-1].'</h1></div>'.Markdown($thePost).$c);
}
